...
#define	AssertNoErrorFail(inMessage, inHandler)						\
if(result != noErr)									\
{													\
printf("%s: %d\n", inMessage, (int)result);		\
if (CurFileInfo) { \
...
//	AssertNoError("Error getting file data info", fail);
	...
	if (theURL == NULL)
		...
		result = 0;
	AssertNoErrorFail("Error opening URL", fail);
	...
	CFRelease(theURL);
	AssertNoErrorFail("Error opening file", fail);
