...
boolean HandleButton( ibutton_t *button ) {
	...
	if ( button->title ) {
		...
		if ( button->buttonFlags & BF_CENTERTEXT ) {
			...
			#ifdef IPAD
			y = button->y + button->drawHeight + 22;
			#else
...
void iphoneDrawNotifyText() {
	...
	glColor4f( 1, 1, 1, f );
	#ifdef IPAD
	iphoneCenterText( 512, 25, 1, notifyText );
	#else
...
void AutomapControls() {
	...
	if ( touchCount == 1 ) {
		...
		#ifdef IPAD
		m_x -= ( t->x - prevX ) * (float)m_w / 1024;
		m_y += ( t->y - prevY ) * (float)m_w / 1024;	
		#else
		...
		#ifdef IPAD
		float	midxDoom = m_x + m_w * midx / 1024;
		float	midyDoom = m_y + m_w * midy / 1024;
		#else 
		...
		#ifdef IPAD
		m_x = midxDoom - m_w * midx / 1024;
		m_y = midyDoom - m_w * midy / 1024;
		#else
...
#ifdef IPAD
float	weaponSelectDrawScale = 1.75f;
#else
...
void iphoneFrame() {
	...
	if ( advancedemo ) {
		...
		static const char *demoNames[27] = { "demo1", "demo2", "demo3", "demo4", "demo5",
		"demo6", "demo7", "demo8", "demo9", "demo10", "demo11", "demo12", "demo13", "demo14", "demo15",	
		"demo16", "demo17", "demo18", "demo19", "demo20", "demo21", "demo22", "demo23", "demo24", "demo25",
		"demo26", "demo27"};
...
void iphoneDrawScreen() {
	...
	if ( automapmode & am_active ) {
		...
		if ( !netgame ) {	// no save game option during net play
			...
			#ifdef IPAD
			if ( NewTextButton( &btnSave, "SAVE", 1024-64-5, 5, 64, 32 ) ) {
			...
			if ( players[consoleplayer].playerstate == PST_DEAD ) {
				...
				if ( !deathmatch && !netgame ) {
					...
					if ( !btnSaved.texture ) {
						// initial setup
						#ifdef IPAD
						SetButtonPicsAndSizes( &btnSaved, "iphone/load_saved.tga", "Saved game", 512 - 48 - 96 - 48 - 96, 292, 96, 96 );
						SetButtonPicsAndSizes( &btnRespawn, "iphone/respawn.tga", "Restart", 512 - 48, 292, 96, 96 );
						SetButtonPicsAndSizes( &btnGear, "iphone/respawn_gear.tga", "Add gear", 512 + 48 + 48 + 96, 292, 96, 96 );
						#else
...
//					iphoneDrawHudControl( &huds.fire );
			...
			#ifdef IPAD
				PK_StretchTexture( PK_FindTexture("iphone/multiplay.tga"), 512 - 96/2, 384-96/2, 96, 96 );
			#else
